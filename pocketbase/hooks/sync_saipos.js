...
// ===========================================================================
// Cron job: sincronização automática diária às 7:00.
// Sincroniza as vendas do dia anterior (ontem, em horário de Brasília/UTC-3)
// usando a MESMA lógica do endpoint manual POST /backend/v1/sync-saipos.
//
// Por que duplicar os helpers aqui (em vez de extrair para um escopo
// compartilhado): o JSVM do PocketBase executa os callbacks de routerAdd e
// cronAdd em um pool de VMs SEPARADO do que os registra. Declarações de
// top-level (funções/const/let/var fora dos callbacks) NÃO são visíveis
// dentro de nenhum callback em runtime (ReferenceError). Extrair os helpers
// para o topo do arquivo, portanto, quebraria TANTO o endpoint manual quanto
// o cron. A abordagem mais confiável é duplicar a lógica inline dentro do
// callback do cron (idêntica à do routerAdd), conforme a convenção do JSVM.
//
// cronAdd NÃO recebe objeto `e` (request event): erros são logados via
// $app.logger() e a execução é abortada com `return` (nunca lança exceção,
// para não quebrar o scheduler do PocketBase). A rota manual continua
// funcionando normalmente — este cron é adicional.
// ===========================================================================
cronAdd('saipos_daily_sync', '0 7 * * *', () => {
  $app.logger().info('Saipos cron: iniciando sincronização diária automática')
  ...
  if (!token) {
    $app
      .logger()
      .error(
        'Saipos cron: token não configurado (SAIPOS_API_TOKEN). Abortando sincronização diária.',
      )
    return
  }
  token = token.replace(/^Bearer\s+/i, '').trim()
  ...
  function mapChannel(raw) {
    ...
    if (CHANNEL_MAP[c]) {
      $app
        .logger()
        .info(
          'Saipos cron channel map: "' + original + '" -> "' + CHANNEL_MAP[c] + '" (match exato)',
        )
      return CHANNEL_MAP[c]
    }
    ...
    if (c.indexOf('telefone') === 0) {
      $app
        .logger()
        .info('Saipos cron channel map: "' + original + '" -> "Telefone" (prefixo "telefone")')
      return 'Telefone'
    }
    ...
    $app.logger().info('Saipos cron channel map: "' + original + '" -> "" (sem correspondência)')
    return ''
  }
  ...
  function extractChannel(entry, unmappedPartners) {
    ...
    if (!psName && !spName && saleType === 1) {
      $app
        .logger()
        .info(
          'Saipos cron fallback Telefone: venda sem parceiro em nenhum campo ' +
            '(partner_sale/partner_delivery/desc_store_partner vazios) com ' +
            'id_sale_type=1 (Delivery) -> "Telefone" | id_sale=' +
            (entry.id_sale || ''),
        )
  ...
  // Versão do requestWithRetry adaptada ao cron: NÃO há objeto `e`, então em
  // vez de devolver `errorResponse: e.json(...)` retorna `{ error: msg }`. O
  // chamador loga o erro e aborta. Lógica de retry (PGRST003, 3 tentativas,
  // 2s entre elas) idêntica ao endpoint manual.
  function requestWithRetry(url, timeoutSecs) {
    var MAX_ATTEMPTS = 3
    var res = null
    for (var attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (
          ...
          return {
            error: 'Timeout: a API do Saipos demorou demais para responder.',
          }
        }
        return { error: 'Erro de conexão com a API do Saipos: ' + errMsg }
      }
      ...
      if (isPoolTimeout) {
        if (attempt < MAX_ATTEMPTS - 1) {
          $app
            .logger()
            .warn(
              'Saipos cron PGRST003 (pool de conexão) na tentativa ' +
                (attempt + 1) +
                '/' +
                MAX_ATTEMPTS +
                ' — retryando em 2s',
        ...
        $app
          .logger()
          .error(
            'Saipos cron PGRST003: pool de conexão esgotado após ' + MAX_ATTEMPTS + ' tentativas',
          )
        return {
          error:
            'A API do Saipos está indisponível no momento (PGRST003: Timed out acquiring connection from connection pool).',
        }
      }
  ...
  // Versão do fetchSegment adaptada ao cron: em vez de `errorResponse:
  // e.json(...)` retorna `{ error: msg }`. Paginação e diagnóstico idênticos
  // ao endpoint manual.
  function fetchSegment(startISO, endISO) {
    var allItems = []
    var offset = 0
    ...
    for (var page = 0; page < maxPages && hasMore; page++) {
      ...
      if (res.statusCode === 401 || res.statusCode === 403) {
        return { error: 'Token Saipos Inválido (HTTP ' + res.statusCode + ')' }
      }
      if (res.statusCode === 404) {
        $app.logger().error('Saipos cron: endpoint não encontrado na API do Saipos', 'url', apiUrl)
        return { error: 'Endpoint não encontrado na API do Saipos: ' + apiUrl }
      }
      if (res.statusCode === 429) {
        var retryAfter = res.headers['Retry-After'] || res.headers['retry-after'] || '60'
        return {
      ...
      if (res.statusCode === 400) {
        ...
        $app.logger().error('Saipos cron 400 no segmento ' + startISO + '..' + endISO + ': ' + b400)
        return {
          error: 'A API do Saipos rejeitou o período (HTTP 400). Detalhe: ' + b400,
        }
      }
      if (res.statusCode !== 200) {
        return { error: 'Erro na API do Saipos (HTTP ' + res.statusCode + ')' }
      }
      ...
      if (page === 0) {
        ...
        $app.logger().warn('Saipos cron diagnóstico: ' + JSON.stringify(segDiagnostic))
      }
  ...
  // --- Cálculo de "ontem" em horário de Brasília (UTC-3) ---
  // O servidor pode estar em UTC; o restaurante está em BRT (UTC-3). O Brasil
  // não usa horário de verão desde 2019, então BRT é sempre UTC-3. Para obter
  // o "agora" em Brasília, subtraímos 3h do instante UTC e lemos os campos
  // getUTC* do Date resultante (eles refletem o wall-clock de BRT). Em
  // seguida subtraímos 1 dia (24h) para ter "ontem" no calendário de BRT.
  // Isso é robusto independentemente do fuso do servidor ou do horário exato
  // em que o cron dispara.
  var now = new Date()
  var brtNow = new Date(now.getTime() - 3 * 60 * 60 * 1000)
  var yesterdayBrt = new Date(brtNow.getTime() - 24 * 60 * 60 * 1000)
  var startDateRaw =
    yesterdayBrt.getUTCFullYear() +
    '-' +
    pad2(yesterdayBrt.getUTCMonth() + 1) +
    '-' +
    pad2(yesterdayBrt.getUTCDate())
  var endDateRaw = startDateRaw

  $app.logger().info('Saipos cron: sincronizando vendas de ' + startDateRaw + ' (ontem BRT)')

  // --- Segmentação do período (idêntica ao endpoint manual; 1 dia = 1 segmento) ---
  var startD = parseStartDate(startDateRaw)
  var endD = parseStartDate(endDateRaw)
  if (!startD || !endD) {
    $app.logger().error('Saipos cron: data de ontem inválida: ' + startDateRaw)
    return
  }
  ...
  for (var si = 0; si < segments.length; si++) {
    ...
    if (segResult.error) {
      $app
        .logger()
        .error(
          'Saipos cron: erro ao buscar segmento ' +
            startISO +
            '..' +
            endISO +
            ': ' +
  ...
  // --- Mapeamento da resposta para daily_sales (idêntico ao endpoint manual) ---
  var groups = {}
  var skipped = 0
  var skippedOtherStore = 0
  var skippedUnrecognizedPartner = 0
      ...
      if (!channel) {
        ...
        if (entry.id_sale_type === 1 && skippedTypeId1Logged < 10) {
          ...
          $app
            .logger()
            .info(
              '[Saipos cron] Venda pulada id_sale_type=1 #' +
                skippedTypeId1Logged +
                ' | id_sale=' +
                (entry.id_sale || '') +
                ' | shift_date=' +
  ...
  // --- Resumo de parceiros (diagnóstico, idêntico ao endpoint manual) ---
  console.log('[Saipos cron] Resumo de parceiros:')
  var _partnerNames = Array.from(partnerSummary)
  if (_partnerNames.length === 0) {
    console.log('[Saipos cron]   (nenhum parceiro encontrado)')
  } else {
    for (var _pi = 0; _pi < _partnerNames.length; _pi++) {
      var _partnerName = _partnerNames[_pi]
      var _mappedChannel = mapChannel(_partnerName)
      console.log('[Saipos cron]   "' + _partnerName + '" -> ' + (_mappedChannel || 'NÃO MAPEADO'))
    }
  }
  $app
    .logger()
    .info('[Saipos cron] Resumo de parceiros: ' + _partnerNames.length + ' valor(es) único(s)')

  // --- Salvamento no banco (idêntico ao endpoint manual) ---
  // Única diferença vs. endpoint manual: no cron não há objeto `e`, então em
  // vez de `return e.json(400, ...)` no primeiro erro de validação, logamos o
  // erro (warn) e SEGUIMOS para o próximo registro — assim o dia é sincronizado
  // o máximo possível de forma autônoma. A lógica de set/campos/upsert é
  // idêntica (orders, revenue, average_ticket, findRecordsByFilter por
  // date+channel).
  var collection = $app.findCollectionByNameOrId('daily_sales')
  var inserted = 0
  var updated = 0
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    try {
      ...
      $app
        .logger()
        .warn(
          'Saipos cron: validação falhou ao salvar daily_sales -> ' +
            'date=' +
            g.date +
            ' | channel=' +
            g.channel +
      ...
      $app
        .logger()
        .error(
          'Saipos cron: falha ao salvar daily_sales (validação): ' +
            errMsg +
            ' | diagnóstico: ' +
            JSON.stringify(firstPageDiagnostic),
        )
  ...
  // --- Resultado logado (inseridos, atualizados, pulados) ---
  $app
    .logger()
    .info(
      'Saipos cron: sincronização de ' +
        startDateRaw +
        ' concluída — inseridos=' +
        inserted +
        ' | atualizados=' +
